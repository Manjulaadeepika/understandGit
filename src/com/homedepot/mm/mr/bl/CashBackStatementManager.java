...
public class CashBackStatementManager {
	...
	private CashBackStatementDTO readCashbackStatementData(final int customerId, final int programId, final short mbrshpAmtTypCd, boolean isSimulateLoadTest) 
		...
		MembershipAmountDTO quarterSummary = MembershipAmountDAO.getQuarterlyMembershipSummary(customerId, programId, mbrshpAmtTypCd, String.valueOf(RAPUtil.getCurrentYear()));
		...
		MembershipLevelDTO customerLevel = MembershipLevelDAO.getMembershipLevel(customerId, programId, lastDayOfQtr);
		//Read the customer transactions for the cashback period.
		List<ReadCustomerTransactionsDTO> transactionList = MembershipTransactionsDAO.getMembershipTransactions(
